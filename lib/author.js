...
exports.delete_process = function (request, response) {
    ...
    request.on('end', function () {
        ...
        db.query(`DELETE
                  FROM topic
                  WHERE author_id = ?`, [post.id], function (error1, result1) {
            if (error1) {
                throw error1;
            }
        })
...
exports.create_process = function (request, response) {
    ...
    request.on('end', function () {
        var post = qs.parse(body);
        db.query(`INSERT INTO author (name, profile, id)
                  VALUES (?, ?, ?)`,
...
exports.home = function (request, response) {
    db.query(`SELECT *
        ...
        db.query(`SELECT *
                  FROM author`, function (error2, authors) {
            var title = 'author';
...
exports.update = function (request, response) {
    ...
    db.query(`SELECT *
        ...
        db.query(`SELECT *
            ...
            db.query(`SELECT *
                      FROM author
                      WHERE id = ?`, queryData.id, function (error3, author) {
                var title = 'author';
                var list = template.list(topics);
                var html = template.html(title, list, `
            ${template.authorTable(authors)}
